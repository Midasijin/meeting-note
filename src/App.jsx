...
function App() {
  ...
  const executeAIAnalysis = async () => {
    ...
    try {
      ...
      const prompt = `
      입력된 [오늘의 회의 녹취록]을 완벽하게 분석하여 회의제목, 회의일시, 본문 정제 결과를 도출해야 합니다.
      특히 제공된 [타 부서들의 비식별 공개 요약 목록 (Public Summaries)]을 면밀히 비교하여, 오늘 회의 내용과 상충되는 일정/업무 충돌이 있는지, 혹은 타 부서의 기존 해결책을 통해 시너지를 낼 수 있는 부분이 있는지 분석하세요.
      반드시 아래의 출력 요구사항을 엄격하게 준수하여 정확한 JSON 객체 하나만 반환하세요. 앞뒤로 마크다운 태그(\`\`\`)를 절대 붙이지 마세요.

      ★출력 포맷 JSON 스키마:
      {
        "extractedDate": "YYYY-MM-DD 형식의 회의 날짜",
        "extractedTitle": "세련된 비즈니스용 회의 제목",
        "aiAttendees": "녹취록 내 대화 참석자 명단 (쉼표 구분)",
        "minutesContent": "샵(#), 별표(*), 대시(-) 같은 마크다운을 절대 쓰지 말고 줄바꿈과 한글 개조식(~함, ~임. ~판단됨.)으로 작성한 대기업 보고서용 요약본",
        "publicSummary": "이 회의록의 보안에 민감한 상세 내용(인명, 극비 소스코드 등)을 철저히 제외하고, 타 부서가 공유받았을 때 일정 충돌이나 업무 연계를 검토할 수 있도록 객관적으로 기술한 1~2줄의 비식별화 요약문",
        "actionItems": [
          {
